...
// OCRで数字を抽出する関数
double extractNumberFromImage(const cv::Mat& thresh) {
...
int main() {
    // 画像の読み込み
    cv::Mat image = cv::imread("../thicknessImage/IMG_8.png");
    if (image.empty()) {
    ...
    try {
        ...
        // cv::Mat thresh = preprocessAndCrop(image, int(width/4), 15, int(width/2 - 15), int(height/2 + 15));
        ...
        // cv::Mat thresh = preprocessAndCrop(image, int(width/4), int(height/3+15), int(width/2-15), int(height/3)); // IMG_3
